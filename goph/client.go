...
// RunCmds starts a new SSH session and runs cmds as a single shell script.
// Later commands share the same shell context as earlier ones.
func (c Client) RunCmds(cmds ...string) ([]byte, error) {
	...
	return RunCmds(sess, cmds...)
}

// RunCmds runs cmds as a single shell script in the provided SSH session.
// Later commands share the same shell context as earlier ones.
func RunCmds(sess *ssh.Session, cmds ...string) ([]byte, error) {
	if sess == nil {
		return nil, errors.New("ssh session is nil")
	}

	script, err := buildScript(cmds...)
	if err != nil {
		return nil, err
	}
...
// RunCmdsContext starts a new SSH session with context and runs cmds as a single shell script.
// Later commands share the same shell context as earlier ones.
func (c Client) RunCmdsContext(ctx context.Context, cmds ...string) ([]byte, error) {
	script, err := buildScript(cmds...)
	if err != nil {
		return nil, err
	}

	return c.RunContext(ctx, script)
}
...
func buildScript(cmds ...string) (string, error) {
	script := make([]string, 0, len(cmds))
