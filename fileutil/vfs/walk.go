...
// a stored list of directory entries.
...
// WalkFiles walks the filesystem tooted at root, calling walkFun gor each file or
// directory inf the filesystem, including root. In addition to FileInfo, it passes an ReadSeeker
// to walkFn for each file it visits.
...
// walkFiles recursively descend path, calling walkFn.
...
func walkFiles(fs http.FileSystem, path string, info os.FileInfo, file http.File, walkFn WalkFilesFunc) error {
	...
	for _, name := range names {
		...
		if err != nil {
			if err = walkFn(filename, nil, nil, err); err != nil && !errors.Is(err, filepath.SkipDir) {
				return err
			}
		} else {
...
// The caller is responsible for the returned file when done.
