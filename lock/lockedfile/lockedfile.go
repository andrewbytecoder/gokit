...
// If flag incudes os.O_WRONLY or os.O_RDWR, the file is locked for writing.
...
func OpenFile(name string, flag int, perm os.FileMode) (*File, error) {
	...
	// 当一个对象被GC之前会触发这个函数，因此被GC之前要将对应的函数设置为nil否则会触发panic，常在close的地方设置为nil
	// 这种方法常用于加锁资源的处理，如果有资源泄露GC就会触发panic
...
// then write-locks it and overwrites it with the given contents.
...
func Transform(name string, t func([]byte) ([]byte, error)) error {
	f, err := Edit(name)
	if err != nil {
		return err
	}
